...
## 五、連接 Typeless (AI 語音輸入)

### 安裝 Typeless
可以使用 `winget` 進行一鍵安裝：
```powershell
winget install -e --id SimplyCA.Typeless
```

### 快捷鍵與口述測試
* **預設快捷鍵**：**`Right Alt`**（空白鍵右側的第一個按鍵）。
* **使用方式**：在可以打字的地方點一下游標 ➡️ 按一下 `Right Alt`（不需長按）➡️ 開始說話 ➡️ 說完後再按一下 `Right Alt` 結束。

### 新手教學卡關排障 (關鍵)
如果因為麥克風偵測問題或快捷鍵衝突，導致軟體卡在「體驗一下」新手教學畫面而「沒有反應」時，請執行以下步驟繞過教學：
1. 強制關閉 Typeless（在工作管理員中結束所有 Typeless 處理程序）。
2. 開啟設定檔目錄：`C:\Users\<使用者>\AppData\Roaming\Typeless.exe\`
3. 修改 `app-onboarding.json` 檔案：
   * 將 `"isCompleted": false` 改為 `"isCompleted": true`
   * 將 `"onboardingTryItPlaygroundIsCompleted": false` 改為 `"onboardingTryItPlaygroundIsCompleted": true`
   * 將 `"onboardingStep"` 與 `"onboardingMaxReachedStep"` 的值都改為 `"completed"`
4. 修改 `app-storage.json` 檔案：
   * 找到 `"windows": { "completed": false }` 欄位，將其改為 `"completed": true`。
5. 重新啟動 Typeless，軟體將會直接進入主程式，繞過教學畫面。

### 麥克風與繁體中文（台灣）設定
* **麥克風孔位置**：ThinkPad 筆電的麥克風孔位於螢幕最上方視訊鏡頭（Webcam）左右兩側的小針孔。
* **輸出為繁體中文**：
  1. 開啟 Typeless 主視窗，點擊上方的 **「設置」**。
  2. 找到最下方的 **「語言變體」 (Language Variants)**，點選右邊的 **「選擇語言變體」** 按鈕。
  3. 在跳出的視窗中將 **「中文 (Chinese)」** 更改為 **「繁體中文（台灣）」** 或 **Traditional Chinese - Taiwan**。
* **功能鍵注意**：ThinkPad 等筆電最上排的功能鍵（如 `F8`）預設為系統功能（如投影），直接按下無法觸發快速鍵，必須按 **`Fn + F8`** 或是開啟 **`FnLock` (Fn + Esc)** 才能直接使用功能鍵。
