...
5. **同步（依 `~/.claude/machine-config.json` 自動決定管道，不用每次問）**：
   - **`machineType: "home"`（家用）→ GitHub + Google Drive 都同步**；若 `googleDriveInstalled:false` 先提醒安裝、本次只走 GitHub。
   - **`machineType: "company"`（公司）→ 只走 GitHub**（Google Drive 被封鎖）。
   - 設定檔不存在 → 問一次「這是家用電腦嗎？(Y/N)」，家用再偵測 Google Drive，寫入設定檔記住，之後不再問。
   - **GitHub**：專案代碼 `git diff` 確認後只 stage 相關檔案（不用 `git add .`）commit + push；第二大腦切到 vault commit + push 到 `my-second-brain`。
   - **Google Drive**：第二大腦 vault 與專案資料夾複製到 `googleDrivePath\SecondBrain`（不存在自動建立）。
